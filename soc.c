 ...
 */
	...
	if (!server){
		sock = setup_client();
		//fprintf(stderr, "CLIENT %d", sock);
	} else{
		sock = setup_server();
		//fprintf(stderr, "SERVER %d", sock);
	}
	main_2();
	return(0);
}
...
setup_server() {
	...
	listen(s, 1);		// listening to a max of 1 pending connections for master socket s
	newsock = s;
	...
	while (newsock = accept(s, (struct sockaddr *) &remote, &len)){
		pthread_create(&thread1, NULL, (void *) &main_2, (void *)"");
...
usage()
{
	...
	fprintf(stderr, "usage: myhttpd [âˆ’d] [âˆ’h] [âˆ’l file] [âˆ’p port] [âˆ’r dir] [âˆ’t time] [âˆ’n threadnum] [âˆ’s sched]\n");
...
parse_buf(char buf[])
{
	...
	if(strcmp(buf,"\n") == 0){ 
		...
		close(sock);
		sock = -1;		// assign sock to -1 to keep the descriptor value	
	}
